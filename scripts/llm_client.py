...
# -*- coding: utf-8 -*-
"""
LLM客户端 - 统一管理豆包/Doubao和DeepSeek API调用
优先使用豆包2.0-Pro，失败后自动切换DeepSeek
"""
import os
import json
import logging
...
class LLMClient:
    """LLM客户端 - 支持豆包主模型 + DeepSeek兜底"""

    def __init__(self, api_key: Optional[str] = None, model: str = DOUBAN_MODEL):
        """
        初始化LLM客户端

        Args:
            api_key: 已废弃，保留兼容性
            model: 已废弃，保留兼容性（始终使用豆包主模型）
        """
        self.douban_api_key = DOUBAN_API_KEY
        self.douban_api_url = DOUBAN_API_URL
        self.douban_model = DOUBAN_MODEL
        ...
        logger.info(f"主模型: 豆包 {self.douban_model}")
        logger.info(f"兜底模型: DeepSeek {self.deepseek_model}")
        if self.douban_api_key:
            logger.info(f"豆包API Key: {self.douban_api_key[:10]}...")
        if self.deepseek_api_key:
            logger.info(f"DeepSeek API Key: {self.deepseek_api_key[:10]}...")

    def _call_douban(self, prompt: str, system_prompt: Optional[str] = None,
                     temperature: Optional[float] = None, max_tokens: Optional[int] = None) -> Optional[str]:
        """调用豆包 2.0-Pro API"""
        headers = {
            "Content-Type": "application/json",
            "Authorization": f"Bearer {self.douban_api_key}"
        }
        ...
        try:
            ...
            logger.warning(f"豆包API调用失败: HTTP {response.status_code}, {response.text[:200]}")
            return None
        except Exception as e:
            logger.warning(f"豆包API请求异常: {e}")
            return None

    def _call_deepseek(self, prompt: str, system_prompt: Optional[str] = None,
                        temperature: Optional[float] = None, max_tokens: Optional[int] = None) -> Optional[str]:
        """调用DeepSeek API（兜底）"""
        headers = {
            "Content-Type": "application/json",
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "choices" in result and len(result["choices"]) > 0:
                    return result["choices"][0]["message"]["content"]
            logger.warning(f"DeepSeek API调用失败: HTTP {response.status_code}, {response.text[:200]}")
            return None
        except Exception as e:
            logger.warning(f"DeepSeek API请求异常: {e}")
            return None
    ...
    def call(self, prompt: str, system_prompt: Optional[str] = None,
        ...
        策略：优先豆包 2.0-Pro，失败后自动切换 DeepSeek
        ...
        Args:
            ...
            max_retries: 最大重试次数（默认3，适用于每个模型）
            retry_delay: 重试间隔秒数（默认3）

        Returns:
            str: LLM回复内容，如果失败返回None
        """
        logger.info(f"开始调用LLM（豆包主模型 -> DeepSeek兜底）...")
        logger.info(f"Prompt长度: {len(prompt)} 字符")

        # Step 1: 尝试豆包 2.0-Pro
        logger.info(f"尝试主模型: 豆包 {self.douban_model}")
        last_error = None
        for attempt in range(1, max_retries + 1):
            result = self._call_douban(prompt, system_prompt, temperature, max_tokens)
            if result:
                logger.info(f"豆包调用成功，回复长度: {len(result)} 字符")
                return result
            last_error = f"豆包第{attempt}次失败"
            if attempt < max_retries:
                logger.info(f"{retry_delay}秒后重试豆包（第{attempt+1}/{max_retries}次）...")
                time.sleep(retry_delay)

        # Step 2: 豆包失败，切换DeepSeek兜底
        logger.info(f"豆包全部失败，切换DeepSeek兜底")
        for attempt in range(1, max_retries + 1):
            result = self._call_deepseek(prompt, system_prompt, temperature, max_tokens)
            if result:
                logger.info(f"DeepSeek调用成功，回复长度: {len(result)} 字符")
                return result
            last_error = f"DeepSeek第{attempt}次失败"
            if attempt < max_retries:
                logger.info(f"{retry_delay}秒后重试DeepSeek（第{attempt+1}/{max_retries}次）...")
                time.sleep(retry_delay)
                retry_delay *= 2  # 指数退避

        logger.error(f"LLM调用最终失败（豆包+DeepSeek均不可用），最后错误: {last_error}")
        return None
...
if __name__ == "__main__":
    # 测试LLM连接
    print("=" * 60)
    print("LLM连接测试（豆包主模型 -> DeepSeek兜底）")
    print("=" * 60)
    if test_llm_connection():
        print("LLM连接测试成功!")
    else:
        print("LLM连接测试失败（豆包和DeepSeek均不可用）")
