...
class SubscriptionAnalyzer:
    ...
    def _generate_collection_details(self, df):
        ...
        # 按计划收款日期 从新到旧排序
        if '计划收款日期' in df_select.columns:
            df_select['计划收款日期'] = pd.to_datetime(df_select['计划收款日期'], errors='coerce')
            df_select = df_select.sort_values('计划收款日期', ascending=False)
        
        # 格式化日期
        if '计划收款日期' in df_select.columns:
            df_select['计划收款日期'] = df_select['计划收款日期'].apply(
                lambda x: x.strftime('%Y-%m-%d') if pd.notna(x) else '-'
            )
